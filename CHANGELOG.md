...
### Added
- First-class Terraform exposure modes: `module-public`, `private`, and `caller-managed`
- Security-group-source ingress rules for private deployments behind caller-managed load balancers
- Explicit exposure outputs (`exposure_kind`, `has_public_endpoint`, `listener_port`)
- Optional NAT Gateway creation in the shared network module

### Changed
- Public-only defaults now derive from `exposure_kind` instead of assuming every host is internet-facing
- Public assignment deployments can disable NAT entirely while keeping private subnet support available
